...
export function HistogramTab() {
  ...
  // Sync local working copy when shared image changes
  useEffect(() => {
    setCurrentImage(originalImage ? cloneImageData(originalImage) : null);
  }, [originalImage]);

  const imageIsGray = currentImage ? isGrayscale(currentImage) : false;
  ...
  const convertToGray = () => {
    ...
    setCurrentImage(toGrayscale(currentImage));
  };
  ...
  const reset = () => {
    if (originalImage) setCurrentImage(cloneImageData(originalImage));
  };
        ...
        <ControlPanel title="Operations">
          <div className="space-y-2">
            <Button
              ...
              disabled={!currentImage || imageIsGray}
              className="w-full"
              size="sm"
              variant="secondary"
            >
